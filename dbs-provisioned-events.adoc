...
[.lead]
dbs.provisioned events are presented when the distributed block store detects that the cluster's overall provisioned capacity has exceeded the slice reserve space threshold, which may require adding more provisioned space or deleting and purging volumes.
