...
[.lead]
dbs.ssl events are presented when Distributed Block Store (DBS) detects that an SSL certificate on a node is nearing expiration, though the system resolves this automatically by updating the certificate.
