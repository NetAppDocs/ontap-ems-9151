...
[.lead]
master.couldnt events are presented when the master service on a node is unable to cast a vote to update the connection status with the current cluster master due to loss of database connectivity.
