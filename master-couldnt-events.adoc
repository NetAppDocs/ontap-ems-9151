...
[.lead]
master.couldnt EMS events occur when ONTAP encounters issues associated with a node's inability to update the cluster master voting or demotion state in the global database.
