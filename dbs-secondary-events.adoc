...
[.lead]
dbs.secondary events are presented when the Distributed Block Store (DBS) detects that a Slice volume secondary write cache has reached a fullness warning threshold, which may require verifying that all nodes and services are online.
