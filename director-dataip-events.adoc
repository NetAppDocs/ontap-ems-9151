...
[.lead]
director.dataip EMS events occur when a data IP address is being reassigned from one node to another in response to either a user request or an error condition on the cluster.
