...
[.lead]
dbs.slice events are presented when a Distributed Block Store (DBS) event relating to slice service operations occurs, such as metadata drive removal, slice reassignment, or snapshot operations.
