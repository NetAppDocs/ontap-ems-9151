...
[.lead]
dbs.discon events are presented when the Distributed Block Store (DBS) is unable to access a SnapMirror endpoint on the network, which may require checking network connectivity and the 1G management network.
