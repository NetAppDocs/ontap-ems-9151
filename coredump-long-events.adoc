...
[.lead]
coredump.long events are presented when the coredump manager thread detects a message queued longer than expected, though system features and performance are not affected and no corrective action is required.
