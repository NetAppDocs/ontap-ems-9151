...
[.lead]
dipmon.dataip events are presented when a data IP address becomes responsive or unresponsive on the node where it is hosted and the state is committed to the Zookeeper database.
