...
[.lead]
dbs.drives events are presented when Distributed Block Store (DBS) has one or more failed drives, which may require checking network connectivity, replacing drives, or verifying authentication key availability.
