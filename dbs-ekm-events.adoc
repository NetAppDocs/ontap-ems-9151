...
[.lead]
dbs.ekm events are presented when Distributed Block Store (DBS) detects an External Key Management (EKM) certificate nearing expiration, which requires renewing the certificate and updating the EKM key server configuration.
