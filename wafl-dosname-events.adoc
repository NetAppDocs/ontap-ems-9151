...
[.lead]
wafl.dosname events are presented when the system cannot correct duplicate dosnames in a directory, which may require renaming affected files from an NFS client and running the wafl dosname node shell command on the directory.
