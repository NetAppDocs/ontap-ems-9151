...
[.lead]
dbs.sw EMS events occur when a Distributed Block Store (DBS) event relating to Software Encryption At Rest occurs, such as master key rekey completion, failure, or extended key rewrapping.
