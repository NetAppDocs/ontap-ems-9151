...
[.lead]
dbs.sw events are presented when a Distributed Block Store (DBS) event relating to Software Encryption At Rest occurs, such as master key rekey completion, failure, or extended rewrapping of drive keys with a new master key.
