...
[.lead]
dbs.maintenance events are presented when a Distributed Block Store (DBS) maintenance mode event occurs, such as a node no longer existing in the cluster.
