...
[.lead]
dbs.remrep events are presented when the Distributed Block Store (DBS) executing remote replication has not reached active state for 6 hours, which requires checking network connectivity between clusters and inspecting slice service logs.
