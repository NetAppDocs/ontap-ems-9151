...
[.lead]
storage.interface events are presented when a storage interface reports a link status change, which may require checking the network interface configuration and attempting to bring it back online.
