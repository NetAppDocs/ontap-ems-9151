...
[.lead]
dbs.network events are presented when distributed block store detects network configuration or connectivity errors on cluster nodes, which may require checking network parameters or replacing faulty hardware components.
