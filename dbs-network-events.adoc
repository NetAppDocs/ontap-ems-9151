...
[.lead]
dbs.network EMS events occur when ONTAP encounters issues associated with Distributed Block Store (DBS) network configuration, connectivity, or MTU size on cluster nodes.
