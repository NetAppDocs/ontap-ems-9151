...
[.lead]
master.try events are presented when a node cannot connect to the current cluster master and attempts to demote it by making an associated global database change.
