...
[.lead]
dbs.schedule events are presented when the Distributed Block Store (DBS) has a scheduled activity that failed, such as a scheduled snapshot creation, which requires checking the scheduler entry for issues.
