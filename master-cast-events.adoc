...
[.lead]
master.cast events are presented when the master service on a node cannot connect to the current cluster master and casts a vote to demote it, potentially triggering promotion of a new cluster master.
