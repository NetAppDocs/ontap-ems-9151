...
[.lead]
dbs.node events are presented when Distributed Block Store (DBS) detects a node with a maintenance mode failure, which requires contacting NetApp technical support.
